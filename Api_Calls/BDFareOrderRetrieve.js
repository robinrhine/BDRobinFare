...
module.exports = internal.BDFareController = class {
  constructor(qs){
    console.log('Initialize BDFare Order Retrieve object');
    this.QS =qs;
  ...
  async  OrderRetrieve() 
  {
            ...
            let {status,statusText,data} = await _onCallBDFareAPIForOrderRetrieve(this.apiURL,this.payload,this.secrect_key);
            if(status === 500)
            {
              ...
              return {
                ...
                return {
                    status:200,
                    statusText:'success',
                    data:data.OrderRetrieveResponse
                  }
            }
...
async function _onCallBDFareAPIForOrderRetrieve (apiURL,payload,apiKey)
{
      ...
      if(res.ok)  // if successfull.
        {
          ...
          if(Responsedata.statusCode === 'OK' && Responsedata.success === true && Responsedata.error === null){
            ...
            oReturn.data={OrderRetrieveResponse : Responsedata.response};
            return oReturn;
